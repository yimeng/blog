...
![HomePulse 家庭数据中枢公开版架构图](https://cdn.jsdelivr.net/gh/yimeng/blog-pic@main/post/2026/homepulse/architecture-public.svg)
...
为了方便公开展示，下面的 Dashboard 截图都做了脱敏：保留面板布局、趋势形态和分析关系，但不展示真实内网地址、密钥、精确时间点、通知正文、站点细节和个人健康数值。
...
![HomePulse 电量分析 Dashboard 公开脱敏视图](https://cdn.jsdelivr.net/gh/yimeng/blog-pic@main/post/2026/homepulse/grafana-power-overview.svg)

这张电量 Dashboard 的重点不是追求某个瞬间的读数，而是把几个问题放在一起看：

- 总进线和分支回路之和是否长期接近；
- 某一路的阶跃、短脉冲或周期波动是否有稳定形态；
- 负载变化是否能和家电状态、传感器变化或人工标注互相印证。

对家庭场景来说，这比单纯画一堆曲线更有用。曲线只告诉我“发生了变化”，事件和标注才开始解释“可能发生了什么”。
...
![HomePulse 设备事件与生活场景 Dashboard 公开脱敏视图](https://cdn.jsdelivr.net/gh/yimeng/blog-pic@main/post/2026/homepulse/grafana-device-scenes.svg)

例如洗漱、洗澡、冲水这些场景，单看某一路功率都容易误判。更稳妥的做法是把多个证据放在同一条时间线上：

- 卫生间照明或换气先出现稳定小功率负载；
- 热水器或水泵在短时间内进入工作状态；
- 温湿度传感器出现滞后变化；
- 人体存在、门锁、冰箱门等离散事件提供额外上下文；
- 最后由人工反馈确认或修正这次推断。

这种方式不要求任何一个传感器“全知全能”。每个信号都可以有噪声，但多个信号在同一个时间窗口里按稳定顺序出现时，可信度就会明显提高。
...
![HomePulse 通勤与健康相关性 Dashboard 公开脱敏视图](https://cdn.jsdelivr.net/gh/yimeng/blog-pic@main/post/2026/homepulse/grafana-commute-health.svg)

这类 Dashboard 最重要的是把“事实”和“估算”分开：刷卡时间、设备状态、运动记录是事实；出门时间、等待时间、场景标签是推断。图表上如果不区分这两类数据，很容易把一个看起来漂亮的数字误认为真实发生过的事情。
...
现在这套系统里，Grafana 主要有几类看板：

- **电量与回路**：看总进线、分支回路、候选负载和覆盖差值；
- **家电与传感器**：看冰箱、热水器、空气、温湿度、门状态等设备事件；
- **通知与通勤**：把交通卡、电话等通知解析成结构化事件；
- **个人健康与生活节律**：观察运动、睡眠、心情和动态健康指标之间的长期关系；
- **场景分析**：把规则、AI 推断和人工反馈放在同一个闭环里。

Home Assistant 则更像“当前家里是什么状态”的入口。它适合承接 MQTT Discovery 生成的实体、处理自动化、暴露安全的控制按钮，而不是保存所有高频历史。高频时序数据更适合进入 GreptimeDB，再由 Grafana 做长期分析。
...
图片也是同样的原则。公开文章里的截图不直接使用原始 Grafana 页面，而是整理成公开脱敏版本：展示“系统如何分析”，隐藏“我具体什么时候做了什么”。这牺牲了一点现场感，但换来了更清晰的安全边界。
