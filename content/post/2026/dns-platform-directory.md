...
- A/AAAA：服务或主机的实际地址；
- CNAME：稳定业务域名与实际部署地址解耦；
- PTR：从 IP 反查主机，用于资产核对和日志排障。
...
| `managed-by` | `gitops`、`terraform`、`ansible` |
| `project` | `satellite` |
| `region` | `bj1`、`sh2` |
...
| 不适合的场景 | 主要原因 | 更合理的系统 |
|---|---|---|
| 完整 CMDB | 数据量、资源关系和权限能力不足 | CMDB/服务目录 |
| 配置中心 | 缓存、大小、版本和安全问题 | Nacos、Consul KV、etcd |
| 实时健康状态 | TTL 无法保证实时性 | 监控系统/平台 API |
| 消息队列、Pub/Sub | 没有推送、ACK 和顺序保证 | NATS、Kafka、MQTT |
| IoT 遥测 | 无可靠性和确认机制 | MQTT、HTTP、CoAP |
| 函数触发 | 查询可能被缓存和重放 | Webhook/事件总线 |
| 权限系统 | 缓存会造成权限变更延迟 | IAM/RBAC |
...
| 组件 | 适合承担的职责 |
|---|---|
| CoreDNS | Kubernetes/etcd 集成、转发、改写、模板和定制发现逻辑 |
| PowerDNS Authoritative | 标准权威 Zone、数据库记录、REST API、DNSSEC 和主从复制 |
| PowerDNS Recursor | 递归解析、缓存、条件转发和策略控制 |
| dnsdist | 统一入口、规则分流、负载均衡、限速与协议接入 |
...
| 规模 | 主要关注点 | 设计重点 |
|---|---|---|
| Homelab | 主机名、缓存、少量服务发现 | 利用 DHCP、dnsmasq 和搜索后缀，让名称代替易变的 IP |
| 小公司 | 项目域名、Owner、自动化、证书和审计 | 分清递归与权威，建立命名规范、管理接口和记录生命周期 |
| 大公司 | 多团队委派、高可用、权限和安全治理 | 让 Zone 委派对应组织边界，建设多节点、RBAC、审计、DNSSEC 和安全策略 |
...
- DNS 负责名称、地址、端口、协议、Owner 和入口发现；
- 平台 API 负责实时状态、完整配置、权限和生命周期；
- CMDB 负责资产详情与资源关系；
- 配置中心负责动态配置；
- 监控系统负责健康判断和告警；
- 消息系统负责事件、遥测和触发。
